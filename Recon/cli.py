...
def parse_args():
    parser = ArgumentParser(formatter_class=RawDescriptionHelpFormatter) # try to maintain alphabetical order a-zA-z
    ...
    # dump args into global namespace
    globals().update(vars(args))
    # some of the global vars now get modified; we must declare them global for
    # the changes to reflect across the module
    global very_verbose
    global verbose
    global list_of_number_of_clones
    global list_of_fold_differences
    global initial_scale_factors
    global test_weights_list
    global qs
    global make_table_of_D_numbers
    global file_in
    global make_error_bars
    global error_bar_fits_directory
    ...
    if make_table_of_D_numbers:
        globals()['list_of_filenames'] = file_in
...
def read_observed_clone_size_distribution_v2(file_in, clone_distribution_in_file, threshold, bin_size = None):
    ...
    with open(file_in,'rU') as f:
        ...
        if clone_distribution_in_file: # Format from literature
            for line in f:
                if line[0] != "#":
                    element_0, element_1 = line.split("\t")
        ...
        try:
            ...
            for line in MLE_file:
                ...
                if line == '\n':
                    ...
                    continue
                try: left_side, right_side = [s.strip() for s in line.split("=", 1)]
                except: continue
                if in_start_params:
    ...
    with open(error_bar_file,'rU') as f:
        for line in f:
            if line.startswith("error_envelope_x_vals_%s" % q):
                error_envelope_x_vals = literal_eval( line.split("=")[1].strip() )
            elif line.startswith("error_envelope_y_vals_%s" % q):
                error_envelope_y_vals = literal_eval( line.split("=")[1].strip() )
...
def output_table_of_D_numbers(precomputed_error_bar_file, D_number_output_file, infiles, qs = [0., 1., 2., float("inf")], write_to_disk = True ):
    ...
    with open(precomputed_error_bar_file,'rU') as f:
        for line in f:
            left_side, right_side = [s.strip() for s in line.split("=")]
...
def plot_resample(fitfile, plotfile, precomputed_error_bar_file, x_max = None, y_max = None, javascript_file = None, q=0.):
    """
    ...
    """
    if not javascript_file:
        javascript_file = resource_path + "/" + javascript_file
